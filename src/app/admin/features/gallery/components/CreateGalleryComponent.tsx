    ...
    try {
      ...
      if (!result.ok || (typeof result.body === "object" && result.body && "success" in result.body && result.body.success === false)) {
        const message = typeof result.body === "object" && result.body && "message" in result.body
          ...
          : "მოხდა შეცდომა სურათის დამატებისას";
      ...
      const message = err instanceof Error ? err.message : "მოხდა შეცდომა სურათის დამატებისას";
      setError(message);
      ...
      >
        <div className="mb-10 text-center lg:text-left">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white mb-4 tracking-tighter">
            ახალი <span className="text-cyan-500">ფოტოს</span> დამატება
          </h2>
          <p className="text-gray-400">
            ატვირთეთ გალერეის ფოტო აღწერითა და კატეგორიით
          </p>
        </div>
        ...
        <form className="space-y-8" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <motion.div variants={itemVariants} className="space-y-2">
              <label className="flex items-center text-sm font-medium text-gray-400 ml-1">
                <ImageIcon className="w-4 h-4 mr-2 text-cyan-500" /> სათაური
              </label>
              <input
                name="title"
                required
          ...
          <motion.div variants={itemVariants} className="space-y-2">
            ...
            <textarea
              ...
              onChange={handleChange}
              rows={3}
              placeholder="მოკლე აღწერა ფოტოს შესახებ..."
