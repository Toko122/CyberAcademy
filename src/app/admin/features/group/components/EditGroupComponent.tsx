...
const EditGroupComponent = ({ member }: EditGroupComponentProps) => {
    ...
    try {
      ...
      if (!result.ok || (typeof result.body === "object" && result.body && "success" in result.body && result.body.success === false)) {
        const message = typeof result.body === "object" && result.body && "message" in result.body
          ...
          : "მოხდა შეცდომა მონაცემების განახლებისას";
      ...
      setTimeout(() => {
        setSuccess(false);
        router.push("/features/group");
        router.refresh();
      }, 1500);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "მოხდა შეცდომა მონაცემების განახლებისას";
      setError(message);
      ...
      >
        <div className="mb-10 text-center lg:text-left">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white mb-4 tracking-tighter">
            წევრის <span className="text-cyan-500">რედაქტირება</span>
          </h2>
          <p className="text-gray-400">
            განაახლეთ ინფორმაცია ადმინისტრაციის ან მასწავლებლის შესახებ
          </p>
        </div>
