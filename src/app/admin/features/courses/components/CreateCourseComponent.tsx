    ...
    try {
      ...
      if (!result.ok || (typeof result.body === "object" && result.body && "success" in result.body && result.body.success === false)) {
        const message = typeof result.body === "object" && result.body && "message" in result.body
          ...
          : "მოხდა შეცდომა კურსის დამატებისას";
      ...
      const message = err instanceof Error ? err.message : "მოხდა შეცდომა კურსის დამატებისას";
      setError(message);
      ...
      >
        <div className="mb-10 text-center lg:text-left">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white mb-4 tracking-tighter">
            ახალი <span className="text-cyan-500">კურსის</span> დამატება
          </h2>
          <p className="text-gray-400">შეავსეთ ინფორმაცია სასწავლო პროგრამის შესახებ</p>
        </div>
        ...
        <form className="space-y-8" onSubmit={handleSubmit}>
          ...
          <motion.div variants={itemVariants} className="pt-4">
            <motion.button
              ...
              className={`w-full py-4 text-white font-black rounded-2xl transition-all duration-300 shadow-lg flex items-center justify-center gap-2 ${loading ? "bg-cyan-600 cursor-not-allowed opacity-70" : "bg-cyan-500 hover:bg-cyan-400 cursor-pointer"
                }`}
            >
              {loading ? (
                <>
